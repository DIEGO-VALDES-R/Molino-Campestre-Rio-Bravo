...
export default async function handler(request: Request) {
  ...
  try {
    ...
    if (!apiKey) {
      return new Response(JSON.stringify({ error: 'Falta configurar GEMINI_API_KEY' }), { status: 500, headers: { 'Content-Type': 'application/json' }});
    }

    const genAI = new GoogleGenerativeAI(apiKey);
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

    const prompt = Actúa como un asesor financiero experto para un negocio familiar.
    ...
    return new Response(JSON.stringify({ error: 'Error generando análisis' }), { status: 500, headers: { 'Content-Type': 'application/json' }});
  }
}
